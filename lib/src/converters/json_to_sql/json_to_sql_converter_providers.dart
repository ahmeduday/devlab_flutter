import 'package:devlab_flutter/src/converters/json_to_sql/helpers/sql_generator.dart' as sql_generator;
...
final sqlOutputProvider = StateProvider<String>((ref) {
  ...
  final fields = ref.watch(fieldListProvider).where((element) => element.enabled).toList();

  final values = ref.watch(valuesProvider);

  final enableCreateTable = ref.watch(enableCreateTableProvider);
  final enableCreateTableIfNotExists = ref.watch(enableCreateTableIfNotExistsProvider);
  ...
  if (enableDropTable) {
    output.writeln(sql_generator.getDropTableScript(tableName, ifExists: enableDropTableIfExists));
  ...
  if (enableCreateTable) {
    output.writeln(
        sql_generator.getCreateTableScript(tableName: tableName, fields: fields, ifNotExists: enableCreateTableIfNotExists));
    output.writeln();
  }

  switch (scriptType) {
    case ScriptType.insert:
      output.writeln(sql_generator.getInsertScript(tableName: tableName, fields: fields, values: values));
      break;
    case ScriptType.update:
      output.writeln(sql_generator.getUpdateScript(tableName: tableName, fields: fields, values: values));
      break;
    case ScriptType.delete:
      output.writeln(sql_generator.getDeleteScript(tableName: tableName, fields: fields, values: values));
      break;
  }
...
final fieldListProvider = StateNotifierProvider<JsonToSqlFieldList, List<TableField>>((ref) {
