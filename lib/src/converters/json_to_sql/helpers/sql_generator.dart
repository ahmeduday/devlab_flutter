...
String getCreateTableScript({required String tableName, required List<TableField> fields, required bool ifNotExists}) {
  ...
  final primaryKeys = fields.where((f) => f.primaryKey).map((k) => k.fieldName).toList();
...
String getInsertScript({required String tableName, required List<TableField> fields, required List<Map<String, dynamic>> values}) {
  final script = StringBuffer();
  ...
  for (var map in values) {
    ...
    script.writeln("INSERT INTO $tableName $insertFields VALUES $insertValues;");
  }
...
String _getUpdateValues(List<TableField> fields, Map<String, dynamic> map) {
  ...
  final nonPrimaryKeyFields = fields.where((f) => !f.primaryKey).map((f) => f.fieldName).toList();

  final nonPrimaryKeyValues = map.entries.where((entry) => nonPrimaryKeyFields.contains(entry.key));
...
String _getWhere(List<TableField> fields, Map<String, dynamic> map) {
  ...
  final primaryKeyFields = fields.where((f) => f.primaryKey).map((f) => f.fieldName).toList();

  final primaryKeyValues = map.entries.where((entry) => primaryKeyFields.contains(entry.key));
...
String getUpdateScript({required String tableName, required List<TableField> fields, required List<Map<String, dynamic>> values}) {
  final script = StringBuffer();
...
String getDeleteScript({required String tableName, required List<TableField> fields, required List<Map<String, dynamic>> values}) {
  final script = StringBuffer();
