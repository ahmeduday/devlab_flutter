...
class TableField {
  ...
  TableField copyWith({String? fieldName, DataType? dataType, int? length, bool? primaryKey, bool? enabled, bool? required}) {
    return TableField(
        fieldId: fieldId,
...
class JsonToSqlFieldList extends StateNotifier<List<TableField>> {
  ...
  void setFieldName({required String fieldId, required String name}) {
    state = [
      for (final field in state)
        if (field.fieldId == fieldId) field.copyWith(fieldName: name) else field,
    ];
  }
  ...
  void setDataType({required String fieldId, required DataType dataType}) {
    state = [
      for (final field in state)
        if (field.fieldId == fieldId) field.copyWith(dataType: dataType) else field,
    ];
  }
  ...
  void setFieldEnabled({required String fieldId, bool? enabled}) {
    state = [
      for (final field in state)
        if (field.fieldId == fieldId) field.copyWith(enabled: enabled) else field,
    ];
  }
  ...
  void setFieldRequired({required String fieldId, bool? required}) {
    state = [
      for (final field in state)
        if (field.fieldId == fieldId) field.copyWith(required: required) else field,
    ];
  }
  ...
  void setFieldPrimaryKey({required String fieldId, bool? primaryKey}) {
    state = [
      for (final field in state)
        if (field.fieldId == fieldId) field.copyWith(primaryKey: primaryKey) else field,
    ];
  }
}
