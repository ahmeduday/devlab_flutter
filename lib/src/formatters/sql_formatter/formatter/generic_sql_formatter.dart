...
import 'package:devlab_flutter/src/formatters/sql_formatter/formatter/sql_formatter.dart';
...
class GenericSqlFormatter implements SqlFormatter {
  ...
  String format(String sql, {int numSpaces = 1}) {
    ...
    List<String> splitByQuotes =
        transformString(sql, [(s) => s.replaceAll(RegExp(r'\s+'), ' '), (s) => s.replaceAll("'", "$sep'")]).split(sep);
    T input = T('', createShiftArr(tab), tab, genArray(splitByQuotes, tab), 0, 0);
    T output = List<int>.generate(input.arr.length, (int i) => i).fold(input, updateOutput);

    return transformString(
        output.str, [(s) => s.replaceAll(RegExp(r'\s+\n'), '\n'), (s) => s.replaceAll(RegExp(r'\n+'), '\n'), (s) => s.trim()]);
  }
  ...
  T updateOutput(T acc, int i) {
    ...
    if (originalEl.contains(RegExp('SELECT|SET'))) {
      arr[i] = originalEl.replaceAll(RegExp(r'\,\s+'), ',\n${acc.tab}${acc.tab}');
    }
    String el = arr[i];
  ...
  List<String> createShiftArr(String tab) => List<String>.generate(100, (int i) => '\n${tab * i}');
  ...
  List<String> splitSql(String str, String tab) {
    return allReplacements(tab).fold(str.replaceAll(RegExp(r'\s+'), ' '), (String acc, Replacement r) {
